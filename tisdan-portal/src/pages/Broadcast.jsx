...
export default function Broadcast() {
  ...
  const genColumns = [
    { key: "title", label: "Title" },
    {
      key: "message",
      label: "Message",
      render: (v) => v?.slice(0, 90) + (v?.length > 90 ? "…" : ""),
    },
        ...
        <>
          <Card style={{ marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 13, color: "#64748b", marginBottom: 18 }}>
              This message will be sent via WhatsApp to every customer in the
              system.
            </p>
            <form onSubmit={sendGeneral}>
              ...
              <Field
                label="Message"
                name="message"
                value={genForm.message}
                onChange={(e) =>
                  setGenForm((f) => ({ ...f, message: e.target.value }))
                }
                required
                rows={4}
          ...
          <Card style={{ marginBottom: 20 }}>
            ...
            <form onSubmit={sendPersonal}>
              <SelectSearch
                label="Customer"
                name="user_id"
                value={perForm.user_id}
                onChange={(name, value) =>
                  setPerForm((f) => ({ ...f, [name]: value }))
                }
                required
                endpoint="/users/"
                request={request}
              ...
              <Field
                ...
                onChange={(e) =>
                  setPerForm((f) => ({ ...f, message: e.target.value }))
                }
                required
                rows={4}
                placeholder="Your message to this customer…"
              />
              <Btn type="submit" disabled={sending}>
